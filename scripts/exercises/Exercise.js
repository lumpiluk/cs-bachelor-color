...
export class Exercise {
    ...
    constructor($container, options) {
         ...
         */
        const actual = $.extend({}, defaults, container_options || {}, options || {});
        ...
        for (let i = 0; i < this.task_types.length; i++) { // (for ... of not working in this case...)
            const element = this.task_types[i];
    ...
    read_options_from_container() {
        ...
        const num_rounds = $c.data("num-rounds");
        if (!isNaN(num_rounds))
        ...
        if ($c.data("task-types") != null) {
            ...
            for (const task_type of options.task_types) {
